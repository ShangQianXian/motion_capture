...
if (-not $SkipBlender) {
    ...
    foreach ($exe in $found) {
        foreach ($test in @('test_enable_addon.py', 'test_mock_retarget.py', 'test_retarget_coordinates.py', 'test_review_workflow.py', 'test_motion_presets.py', 'test_orientations.py')) {
