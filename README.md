# Motion Capture for Rigify (v0.3.3)

v0.3.3 加入**足部落点校正**（重定向后按素材脚踝位置重解大小腿，实测把脚踝偏移从 11.6/4.8 cm 降到 3.2/2.8 cm）、重投影诊断与深度主导告警，以及 **二维输入尺度** UI 选项。核查发现三维结果与素材二维检测在检测噪声范围内一致（重投影中位差 5.5 px，检测自身抖动 4.3 px），**没有**加入会进一步"修正"三维的阶段。核查同时定位了偏移的真正来源——**目标骨架与素材人物的骨长比例不一致**（默认 Rigify 骨架实测：方向误差 0.0°，脚踝落点却偏 6.5 cm）。数据、作废结论与复测方法见 [精度核查报告](docs/V0_3_3_PRECISION.md)。
...
- **重定向只传递方向，不传递骨长**：目标骨架的骨段长度决定末端位置。若目标骨架与素材人物比例不同，脚、手会偏离素材位置（方向仍然正确）。默认 **足部落点校正** 开启后会重解大小腿把脚踝拉回素材位置，代价是腿的朝向变约 6°；关掉则只传方向。用 `tools/validate_rig_proportions.py --positions` 在 Blender 中复测自己项目的骨架（同时报出两种配置的数字）。
- **单目沿相机光轴方向的位移不可观测**。`diagnostics.reprojection.depth_dominated_joints` 会列出"位移主要落在光轴方向"的关节：这些关节的横向落点来自模型先验而非素材，核对时请结合正面／背面一起看。
- 重投影残差是整条链路的精度天花板。`diagnostics.reprojection.reprojection` 给出逐关节数值；它接近二维检测自身抖动时，继续"修正"三维只会拟合噪声。
