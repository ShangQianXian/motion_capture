...
v0.3.3 加入重投影诊断与深度主导告警，并实测了 lifter 的输入归一化收益。核查发现：当前链路的三维结果与素材二维检测在检测噪声范围内一致（重投影中位差 5.5 px，检测自身抖动 4.3 px），**没有**加入会进一步"修正"三维的阶段；新增 **二维输入尺度** UI 选项（默认“跟随画面比例”，人物在画面里偏小时改用“规范尺度”）。核查同时定位了"脚部位置偏移"的真正来源——**目标骨架与素材人物的骨长比例不一致**（默认 Rigify 骨架实测高 29%，左踝累计偏移 6.5 cm，而方向误差为 0.0°）。数据、作废结论与复测方法见 [精度核查报告](docs/V0_3_3_PRECISION.md)。
...
- **重定向只传递方向，不传递骨长**：目标骨架的骨段长度决定末端位置。若目标骨架与素材人物比例不同，脚、手会偏离素材位置（方向仍然正确）。默认 Rigify 骨架实测：方向误差 0.0°，但左踝累计偏 6.5 cm、右踝 7.3 cm。用 `tools/validate_rig_proportions.py --positions` 在 Blender 中复测自己项目的骨架。
